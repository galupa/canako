...
func (c *BaseController) SetLogin(user *models.User) {
	c.Controller.SetSession("userinfo", user.Id)
}

func (c *BaseController) LoginPath() string {
	return c.Controller.URLFor("LoginController.Login")
}
...
func (c *BaseController) BuildRequestUrl(uri string) string {
	if uri == "" {
		uri = c.Controller.Ctx.Input.URI()
	}
	return fmt.Sprintf("%s:%d%s",
		c.Controller.Ctx.Input.Site(), c.Ctx.Input.Port(), uri)
}
